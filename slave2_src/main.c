...
static void nvic_setup(void)
{
	...
	/* Without this the I2C2 interrupt routine will never be called. */
...
void i2c2_ev_isr(void)
{
	...
	else if (sr1 & I2C_SR1_RxNE)
	{
		if (timer.mode == 0 || timer.mode == 1) {
			...
			timer.offset[reading++] = i2c_get_data(I2C2);
		} else {
	...
	else if ((sr1 & I2C_SR1_TxE) && !(sr1 & I2C_SR1_BTF))
	{
		if (timer.mode == 0 || timer.mode == 1) {
			...
			i2c_send_data(I2C2, *(buf + bufKey++));
		} else {
			i2c_send_data(I2C2, *(orientation + orienKey++));
		}
	}
